...
CREATE OR ALTER PROCEDURE GD.PA_ActualizarCategoria
    @pN_Id INT,
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SET @pC_Comando = 'La categoría con el Id '+ + CAST(@pN_Id AS NVARCHAR(10)) +' no existe.';
        ...
        BEGIN
			ROLLBACK TRANSACTION;
            SET @pC_Comando = 'Ya existe una categoría con el mismo nombre: ' +@pC_Nombre;
            EXEC GD.PA_InsertarBitacora
                @pN_UsuarioID = @pN_UsuarioID,
                @pC_Operacion = @pC_Operacion,
                @pC_Comando = @pC_Comando,
...
CREATE OR ALTER PROCEDURE GD.PA_InsertarCategoria
    @pC_Nombre NVARCHAR(255),
    @pC_Descripcion NVARCHAR(500),
...
CREATE OR ALTER PROCEDURE GD.PA_EliminarCategoria
    @pN_Id INT,
    @pN_UsuarioID INT, -- ID del usuario
    @pN_OficinaID INT  -- ID de la oficina
AS
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
			ROLLBACK TRANSACTION;
            SET @pC_Comando = 'No se puede eliminar la categoria con id ' + CAST(@pN_Id AS NVARCHAR(10)) +' porque existen documentos asociados que no han sido eliminados.';
            EXEC GD.PA_InsertarBitacora 
                @pN_UsuarioID = @pN_UsuarioID,
                @pC_Operacion = @pC_Operacion,
                @pC_Comando = @pC_Comando,
...
CREATE OR ALTER PROCEDURE GD.PA_ListarCategorias
AS
BEGIN
...
CREATE OR ALTER PROCEDURE GD.PA_ObtenerCategoriaPorId
    @pN_Id INT
AS
BEGIN
    -- Validar que la categoría exista
