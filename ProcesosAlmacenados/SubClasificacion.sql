...
CREATE OR ALTER PROCEDURE GD.PA_InsertarSubclasificacion
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @pN_ClasificacionID IS NOT NULL AND NOT EXISTS (SELECT 1 FROM GD.TGESTORDOCUMENTAL_Clasificacion WHERE TN_Id = @pN_ClasificacionID AND TB_Eliminado = 0)
        BEGIN
        ...
        IF EXISTS (SELECT 1 FROM GD.TGESTORDOCUMENTAL_Subclasificacion WHERE TC_Nombre = @pC_Nombre AND TN_ClasificacionID = @pN_ClasificacionID AND TB_Eliminado = 0)
        BEGIN
			ROLLBACK TRANSACTION;
...
CREATE OR ALTER PROCEDURE GD.PA_ActualizarSubclasificacion
    @pN_Id INT,
    @pC_Nombre NVARCHAR(255),
    @pC_Descripcion NVARCHAR(500),
    @pB_Eliminado BIT,
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM GD.TGESTORDOCUMENTAL_Clasificacion WHERE TN_Id = @pN_ClasificacionID AND TB_Eliminado = 0)
            BEGIN
				ROLLBACK TRANSACTION;
                SET @pC_Comando = 'La clasificacion con el Id '+ CAST(@pN_ClasificacionID AS NVARCHAR(10)) +' no existe.';
                EXEC GD.PA_InsertarBitacora 
        ...
        IF EXISTS (SELECT 1 FROM GD.TGESTORDOCUMENTAL_Subclasificacion WHERE TC_Nombre = @pC_Nombre AND TN_ClasificacionID = @pN_ClasificacionID AND TN_Id != @pN_Id AND TB_Eliminado = 0)
        BEGIN
			ROLLBACK TRANSACTION;
...
CREATE OR ALTER PROCEDURE GD.PA_EliminarSubclasificacion
    @pN_Id INT,
	@pN_UsuarioID INT, -- ID del usuario
    @pN_OficinaID INT  -- ID de la oficina
AS
BEGIN
    ...
    BEGIN TRY
        ...
        WHERE TN_Id = @pN_Id;
		 ...
		 SET @pC_Comando = 'Eliminar SUBCLASIFICACION con ID ' + CAST(@pN_Id AS NVARCHAR(10));
            EXEC GD.PA_InsertarBitacora 
                @pN_UsuarioID = @pN_UsuarioID,
                @pC_Operacion = @pC_Operacion,
                @pC_Comando = @pC_Comando,
...
CREATE OR ALTER PROCEDURE GD.PA_ListarSubclasificaciones
AS
BEGIN
    SELECT TN_Id as Id,
	TC_Nombre as Nombre,
...
CREATE OR ALTER PROCEDURE GD.PA_ObtenerSubclasificacionPorId
    @pN_Id INT
AS
BEGIN
    -- Validar que la subclasificacion exista
