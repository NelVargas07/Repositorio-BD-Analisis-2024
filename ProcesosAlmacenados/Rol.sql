...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Validar que el nombre no est� en uso por otro rol activo
        IF EXISTS (SELECT 1 FROM SC.TGESTORDOCUMENTAL_Rol WHERE TC_Nombre = @pC_Nombre AND TN_Id <> @pN_Id AND TB_Activo = 1)
        BEGIN
            ROLLBACK;
            RETURN 1; -- Indica que el nombre ya est� en uso por otro rol
        END
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Validar que el rol no est� asignado a ning�n usuario
